...
c_is = (tc, assert, exp) -> (c) -> tc.runTest -> assert.eql exp, c
...
exports['can pipe count'] = asyncTest 1, (beforeExit, assert) ->
    @service.query(older_emps) #{select: ['*'], from: 'Employee'})
        .then(@service.count)
        .fail(@fail)
        .done c_is @, assert, 46

exports['can lift arguments into query'] = asyncTest 1, (beforeExit, assert) ->
    @service.count(older_emps).fail(@fail).done c_is @, assert, 46
